...
class TestIsothermalCatalystInputOutputOptions():
    ...
    def test_load_model_state(self):
        ...
        assert pytest.approx(test3.model.C["NH3","Unaged","250C",4.5,17.75].value, 1e-3) == 6.53038e-06
        assert pytest.approx(test3.model.q["q1","Unaged","250C",4,17.75].value, 1e-3) == 0.108787366
        assert pytest.approx(test3.model.S["S1","Unaged","250C",2,17.75].value, 1e-3) == 2.93497636821e-08
        ...
        assert test4.model.t.at(1) == 7.75
        assert test4.model.t.at(2) == 8.75
        assert test4.model.t.at(3) == 9.75
        assert test4.model.t.at(4) == 10.75

        assert pytest.approx(test4.model.Cb["NH3","Unaged","250C",0,7.75].value, 1e-3) == 5.90E-06
        assert pytest.approx(test4.model.C["NH3","Unaged","250C",4.5,7.75].value, 1e-3) == 6.0050979794743536e-12
        assert pytest.approx(test4.model.q["q1","Unaged","250C",4,7.75].value, 1e-3) == 1.3559411569936318e-05
        assert pytest.approx(test4.model.S["S1","Unaged","250C",2,7.75].value, 1e-3) == 0.10485043893178377
        ...
        assert pytest.approx(test5.model.q["q1","Unaged","250C",0.05,9].value, 1e-3) == 0.054099103442522944
