...
# Reference or inlet/outlet terms (vary in time, but not in space)
    [./T_o]    # Reference temperature (and inlet/outlet temperature) for average constant density
        order = FIRST
        family = MONOMIAL
        initial_condition = 773.15    #K
    [../]
    ...
    [./CO2_o]
        order = FIRST
        family = MONOMIAL
        initial_condition = 1E-6    #mol/m^3
    [../]
...
# Calculated average or reference properties from GasProperties
    [./rho_o]
        order = FIRST
        family = MONOMIAL
        initial_condition = 0.46       #kg/m^3
    [../]
...
[AuxKernels]
    ...
    [./ref_dens_calc]
        type = AuxAvgGasDensity
        variable = rho_o
        temperature = T_o
        pressure = P_o
        gases = 'N2_o O2_o CO2_o'
    [../]
