...
# We are declaring an aux variable for inlet velocity magnitude
#   This will be used to set the true velocity at the BC
[AuxVariables]
    [./vel_in]
        order = FIRST
        family = MONOMIAL
...
#   Every block must have an active kernel. Here, we place
#   a dummy kernel for the time being.
  [./inner_dot]
...
[BCs]
  # Force no slip at the obstruction
  [./x_no_slip_obj]
    type = DirichletBC
    variable = vel_x
    boundary = 'object'
  ...
  #
  # As a result, it allows free slip at the walls.
