...
int
initialize_data(int N, MIXED_GAS * gas_dat)
{
  int success = 0;
  if (N <= 0)
  {
...
int
set_variables(
    double PT, double T, double us, double L, std::vector<double> & y, MIXED_GAS * gas_dat)
{
  int success = 0;
...
int
calculate_properties(MIXED_GAS * gas_dat, bool is_ideal_gas, double CT)
{
  int success = 0;
  ...
  if (is_ideal_gas == true)
  {
    ...
    {
      gas_dat->species_dat[0].density =
          CE3(gas_dat->total_pressure * gas_dat->species_dat[0].molecular_weight,
              gas_dat->gas_temperature);
      gas_dat->species_dat[0].dynamic_viscosity = Mu(gas_dat->species_dat[0].Sutherland_Viscosity,
                                                     gas_dat->species_dat[0].Sutherland_Temp,
                                                     gas_dat->species_dat[0].Sutherland_Const,
                                                     gas_dat->gas_temperature);
      gas_dat->binary_diffusion.edit(
          0, 0, D_ii(gas_dat->species_dat[0].density, gas_dat->species_dat[0].dynamic_viscosity));
      ...
      gas_dat->Reynolds =
          ReNum(gas_dat->velocity, gas_dat->char_length, gas_dat->kinematic_viscosity);
      gas_dat->species_dat[0].Schmidt =
          ScNum(gas_dat->kinematic_viscosity, gas_dat->species_dat[0].molecular_diffusion);
    }
    else
    {
      ...
      {
        gas_dat->species_dat[i].density =
            CE3(gas_dat->total_pressure * gas_dat->species_dat[i].molecular_weight,
                gas_dat->gas_temperature);
        gas_dat->species_dat[i].dynamic_viscosity = Mu(gas_dat->species_dat[i].Sutherland_Viscosity,
                                                       gas_dat->species_dat[i].Sutherland_Temp,
                                                       gas_dat->species_dat[i].Sutherland_Const,
                                                       gas_dat->gas_temperature);
        ...
        for (int j = 0; j <= i; j++)
        {
          ...
          if (j == i)
            gas_dat->binary_diffusion.edit(
                i,
                j,
                D_ii(gas_dat->species_dat[i].density, gas_dat->species_dat[i].dynamic_viscosity));
          else
          {
            // Calculate upper triangular portion
            gas_dat->binary_diffusion.edit(i,
                                           j,
                                           D_ij(gas_dat->species_dat[i].molecular_weight,
                                                gas_dat->species_dat[j].molecular_weight,
                                                gas_dat->species_dat[i].density,
                                                gas_dat->species_dat[j].density,
                                                gas_dat->species_dat[i].dynamic_viscosity,
                                                gas_dat->species_dat[j].dynamic_viscosity));
        ...
        // Additive Properties
        gas_dat->total_molecular_weight =
            gas_dat->total_molecular_weight +
            (gas_dat->molefraction[i] * gas_dat->species_dat[i].molecular_weight);
        gas_dat->total_specific_heat =
            gas_dat->total_specific_heat +
            (gas_dat->molefraction[i] * gas_dat->species_dat[i].specific_heat);
      }

      // Calculate total density
      gas_dat->total_density =
          CE3(gas_dat->total_pressure * gas_dat->total_molecular_weight, gas_dat->gas_temperature);
      ...
      for (int i = 0; i < gas_dat->N; i++)
      {
        ...
        for (int j = 0; j < gas_dat->N; j++)
        {
          if (j != i)
          {
            ...
            muT_sum = muT_sum + (gas_dat->molefraction[j] /
                                 Dp_ij(gas_dat->binary_diffusion(i, j), gas_dat->total_pressure));
          }
        }
        ...
        if (gas_dat->molefraction[i] != 0.0)
          gas_dat->total_dyn_vis =
              gas_dat->total_dyn_vis +
              (gas_dat->species_dat[i].dynamic_viscosity /
               (1.0 + ((113.65 * PSI(gas_dat->gas_temperature) *
                        gas_dat->species_dat[i].dynamic_viscosity * gas_dat->gas_temperature) /
                       (gas_dat->molefraction[i] * gas_dat->species_dat[i].molecular_weight)) *
                          muT_sum));
      }
      ...
      gas_dat->Reynolds =
          ReNum(gas_dat->velocity, gas_dat->char_length, gas_dat->kinematic_viscosity);
      for (int i = 0; i < gas_dat->N; i++)
        gas_dat->species_dat[i].Schmidt =
            ScNum(gas_dat->kinematic_viscosity, gas_dat->species_dat[i].molecular_diffusion);
    }
  }
  ...
  else
  {
    ...
    else
    {
      ...
      for (int i = 0; i < gas_dat->N; i++)
      {
        ...
        //-----------------------------------------------------------------
        gas_dat->species_dat[i].density =
            gas_dat->molefraction[i] * gas_dat->species_dat[i].molecular_weight * CT / 1000.0;
        gas_dat->species_dat[i].dynamic_viscosity = Mu(gas_dat->species_dat[i].Sutherland_Viscosity,
                                                       gas_dat->species_dat[i].Sutherland_Temp,
                                                       gas_dat->species_dat[i].Sutherland_Const,
                                                       gas_dat->gas_temperature);
...
EGRET_TESTS()
{
  ...
  do
  {
    ...
    std::cout << "\nO2 Film Mass Transfer Coeff (cm/s): "
              << FilmMTCoeff(dat.species_dat[1].molecular_diffusion,
                             dat.char_length,
                             dat.Reynolds,
                             dat.species_dat[1].Schmidt)
              << std::endl;
