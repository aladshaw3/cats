...
InputParameters
CoupledVariableFluxBC::validParams()
{
...
Real
CoupledVariableFluxBC::computeQpResidual()
{
  _flux(0) = _fx[_qp];
  _flux(1) = _fy[_qp];
  _flux(2) = _fz[_qp];
...
Real
CoupledVariableFluxBC::computeQpJacobian()
{
  return 0.0;
}

Real
CoupledVariableFluxBC::computeQpOffDiagJacobian(unsigned int jvar)
{
  _flux(0) = _fx[_qp];
  _flux(1) = _fy[_qp];
  _flux(2) = _fz[_qp];
