...
InputParameters DGDiffuseFlowMassFluxBC::validParams()
{
...
Real DGDiffuseFlowMassFluxBC::computeQpResidual()
{
  _u_input = _input[_qp];
	_velocity(0)=_ux[_qp];
	_velocity(1)=_uy[_qp];
...
Real DGDiffuseFlowMassFluxBC::computeQpJacobian()
{
  _u_input = _input[_qp];
	_velocity(0)=_ux[_qp];
	_velocity(1)=_uy[_qp];
...
Real DGDiffuseFlowMassFluxBC::computeQpOffDiagJacobian(unsigned int jvar)
{
  _u_input = _input[_qp];
	_velocity(0)=_ux[_qp];
	_velocity(1)=_uy[_qp];
