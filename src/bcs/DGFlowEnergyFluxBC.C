...
InputParameters
DGFlowEnergyFluxBC::validParams()
{
...
Real
DGFlowEnergyFluxBC::computeQpResidual()
{
  _velocity(0) = _ux[_qp];
  _velocity(1) = _uy[_qp];
  _velocity(2) = _uz[_qp];
...
Real
DGFlowEnergyFluxBC::computeQpJacobian()
{
  _velocity(0) = _ux[_qp];
  _velocity(1) = _uy[_qp];
  _velocity(2) = _uz[_qp];
...
Real
DGFlowEnergyFluxBC::computeQpOffDiagJacobian(unsigned int jvar)
{
  _velocity(0) = _ux[_qp];
  _velocity(1) = _uy[_qp];
  _velocity(2) = _uz[_qp];
  ...
  if (jvar == _density_var)
  {
    ...
    {
      r += _test[_i][_qp] * _inlet_temp[_qp] * _porosity[_qp] * _phi[_j][_qp] * _specheat[_qp] *
           (_velocity * _normals[_qp]) * _porosity[_qp];
    }
    return r;
  }
  ...
  if (jvar == _specheat_var)
  {
    ...
    {
      r += _test[_i][_qp] * _inlet_temp[_qp] * _porosity[_qp] * _phi[_j][_qp] * _density[_qp] *
           (_velocity * _normals[_qp]) * _porosity[_qp];
    }
    return r;
  }
  ...
  if (jvar == _inlet_temp_var)
  {
    ...
    {
      r += _test[_i][_qp] * _density[_qp] * _porosity[_qp] * _phi[_j][_qp] * _specheat[_qp] *
           (_velocity * _normals[_qp]) * _porosity[_qp];
    }
    return r;
  }
