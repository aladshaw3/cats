...
InputParameters
INSNormalFlowBC::validParams()
{
  InputParameters params = IntegratedBC::validParams();
  params.addParam<Real>(
      "u_dot_n", 0.0, "Value of the dot product of velocity and normals at boundary");
...
Real
INSNormalFlowBC::computeQpResidual()
{
  _velocity(0) = _ux[_qp];
  _velocity(1) = _uy[_qp];
  _velocity(2) = _uz[_qp];
...
Real
INSNormalFlowBC::computeQpJacobian()
{
  _velocity(0) = _ux[_qp];
  _velocity(1) = _uy[_qp];
  _velocity(2) = _uz[_qp];
...
Real
INSNormalFlowBC::computeQpOffDiagJacobian(unsigned int jvar)
{
  _velocity(0) = _ux[_qp];
  _velocity(1) = _uy[_qp];
  _velocity(2) = _uz[_qp];
