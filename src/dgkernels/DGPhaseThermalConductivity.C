...
InputParameters
DGPhaseThermalConductivity::validParams()
{
...
Real
DGPhaseThermalConductivity::computeQpResidual(Moose::DGResidualType type)
{
  ...
  switch (type)
  {
    case Moose::Element:
      r -= 0.5 *
           (_Diffusion * _volfrac[_qp] * _grad_temp[_qp] * _normals[_qp] +
    ...
    case Moose::Neighbor:
      ...
      r += _epsilon * 0.5 * (_temp[_qp] - _temp_neighbor[_qp]) * _Diffusion_neighbor *
           _volfrac[_qp] * _grad_test_neighbor[_i][_qp] * _normals[_qp];
...
Real DGPhaseThermalConductivity::computeQpJacobian(Moose::DGJacobianType /* type */) { return 0.0; }

Real
DGPhaseThermalConductivity::computeQpOffDiagJacobian(Moose::DGJacobianType type, unsigned int jvar)
{
  _Diffusion(0, 0) = _Dx[_qp];
  _Diffusion(0, 1) = 0.0;
  _Diffusion(0, 2) = 0.0;
  ...
  if (jvar == _Dx_var)
  {
    ...
    switch (type)
    {
      ...
      case Moose::ElementElement:
        r -= 0.5 *
             (_phi[_j][_qp] * _volfrac[_qp] * _grad_temp[_qp](0) * _normals[_qp](0) +
              _phi_neighbor[_j][_qp] * _volfrac[_qp] * _grad_temp_neighbor[_qp](0) *
                  _normals[_qp](0)) *
             _test[_i][_qp];
      ...
      case Moose::NeighborElement:
        ...
        r += _epsilon * 0.5 * (_temp[_qp] - _temp_neighbor[_qp]) * _phi_neighbor[_j][_qp] *
             _volfrac[_qp] * _grad_test_neighbor[_i][_qp](0) * _normals[_qp](0);
        break;
  ...
  if (jvar == _Dy_var)
  {
    ...
    switch (type)
    {
      ...
      case Moose::ElementElement:
        r -= 0.5 *
             (_phi[_j][_qp] * _volfrac[_qp] * _grad_temp[_qp](1) * _normals[_qp](1) +
              _phi_neighbor[_j][_qp] * _volfrac[_qp] * _grad_temp_neighbor[_qp](1) *
                  _normals[_qp](1)) *
             _test[_i][_qp];
        r += _epsilon * 0.5 * (_temp[_qp] - _temp_neighbor[_qp]) * _phi[_j][_qp] * _volfrac[_qp] *
      ...
      case Moose::NeighborElement:
        r += 0.5 *
             ...
             _test_neighbor[_i][_qp];
        r += _epsilon * 0.5 * (_temp[_qp] - _temp_neighbor[_qp]) * _phi_neighbor[_j][_qp] *
             _volfrac[_qp] * _grad_test_neighbor[_i][_qp](1) * _normals[_qp](1);
        break;
  ...
  if (jvar == _Dz_var)
  {
    ...
    switch (type)
    {
      ...
      case Moose::ElementElement:
        r -= 0.5 *
             (_phi[_j][_qp] * _volfrac[_qp] * _grad_temp[_qp](2) * _normals[_qp](2) +
              _phi_neighbor[_j][_qp] * _volfrac[_qp] * _grad_temp_neighbor[_qp](2) *
                  _normals[_qp](2)) *
             _test[_i][_qp];
        r += _epsilon * 0.5 * (_temp[_qp] - _temp_neighbor[_qp]) * _phi[_j][_qp] * _volfrac[_qp] *
      ...
      case Moose::NeighborElement:
        r += 0.5 *
             ...
             _test_neighbor[_i][_qp];
        r += _epsilon * 0.5 * (_temp[_qp] - _temp_neighbor[_qp]) * _phi_neighbor[_j][_qp] *
             _volfrac[_qp] * _grad_test_neighbor[_i][_qp](2) * _normals[_qp](2);
        break;
  ...
  if (jvar == _temp_id)
  {
    ...
    switch (type)
    {
      ...
      case Moose::ElementElement:
        ...
        r += _epsilon * 0.5 * _phi[_j][_qp] * _Diffusion * _volfrac[_qp] * _grad_test[_i][_qp] *
             _normals[_qp];
        r += _sigma / h_elem * _phi[_j][_qp] * _test[_i][_qp];
        break;

      case Moose::ElementNeighbor:
        r -= 0.5 * _Diffusion_neighbor * _volfrac[_qp] * _grad_phi_neighbor[_j][_qp] *
             _normals[_qp] * _test[_i][_qp];
        r += _epsilon * 0.5 * -_phi_neighbor[_j][_qp] * _Diffusion * _volfrac[_qp] *
             _grad_test[_i][_qp] * _normals[_qp];
        r += _sigma / h_elem * -_phi_neighbor[_j][_qp] * _test[_i][_qp];
        break;

      case Moose::NeighborElement:
        r += 0.5 * _Diffusion * _volfrac[_qp] * _grad_phi[_j][_qp] * _normals[_qp] *
             _test_neighbor[_i][_qp];
        r += _epsilon * 0.5 * _phi[_j][_qp] * _Diffusion_neighbor * _volfrac[_qp] *
