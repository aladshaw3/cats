 ...
 *    \details This file creates a discontinous Galerkin kernel for thermal conductivity in a given
 * domain that has a variable diffusivity. The diffusivity is represented by a set of non-linear
 * variables in the x, y, and z directions (in the case of anisotropic diffusion).
 *
...
InputParameters
DGThermalConductivity::validParams()
{
...
Real
DGThermalConductivity::computeQpResidual(Moose::DGResidualType type)
{
  ...
  switch (type)
  {
    case Moose::Element:
      r -= 0.5 *
           (_Diffusion * _grad_temp[_qp] * _normals[_qp] +
...
Real DGThermalConductivity::computeQpJacobian(Moose::DGJacobianType /* type */) { return 0.0; }

Real
DGThermalConductivity::computeQpOffDiagJacobian(Moose::DGJacobianType type, unsigned int jvar)
{
  _Diffusion(0, 0) = _Dx[_qp];
  _Diffusion(0, 1) = 0.0;
  _Diffusion(0, 2) = 0.0;
  ...
  if (jvar == _Dx_var)
  {
    ...
    switch (type)
    {
      ...
      case Moose::ElementElement:
        r -= 0.5 *
             (_phi[_j][_qp] * _grad_temp[_qp](0) * _normals[_qp](0) +
              _phi_neighbor[_j][_qp] * _grad_temp_neighbor[_qp](0) * _normals[_qp](0)) *
             _test[_i][_qp];
        r += _epsilon * 0.5 * (_temp[_qp] - _temp_neighbor[_qp]) * _phi[_j][_qp] *
  ...
  if (jvar == _Dy_var)
  {
    ...
    switch (type)
    {
      ...
      case Moose::ElementElement:
        r -= 0.5 *
             (_phi[_j][_qp] * _grad_temp[_qp](1) * _normals[_qp](1) +
  ...
  if (jvar == _Dz_var)
  {
    ...
    switch (type)
    {
      ...
      case Moose::ElementElement:
        r -= 0.5 *
             (_phi[_j][_qp] * _grad_temp[_qp](2) * _normals[_qp](2) +
