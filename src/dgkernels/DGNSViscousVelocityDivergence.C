 ...
 *	\details This file creates a discontinous Galerkin kernel for the viscous velocity divergence
 *term of the Navier-Stokes equation in a given domain. This term will be zero in the case of
 *      incompressible flow, but will be non-zero for compressible flow. As such, user can invoke
 *this kernel under either circumstance such that the physics can naturally swap from incompressible
...
InputParameters
DGNSViscousVelocityDivergence::validParams()
{
  ...
  params.addRequiredCoupledVar(
      "ux", "Variable for velocity in x-direction [NOTE: Cannot be defaulted to a single value] ");
  params.addRequiredCoupledVar(
      "uy", "Variable for velocity in y-direction [NOTE: Cannot be defaulted to a single value] ");
  params.addRequiredCoupledVar(
      "uz", "Variable for velocity in z-direction [NOTE: Cannot be defaulted to a single value] ");
...
Real
DGNSViscousVelocityDivergence::computeQpResidual(Moose::DGResidualType type)
{
  Real r = 0;
  ...
  switch (type)
  {
    case Moose::Element:
      ...
      r -= 0.5 *
           ((1.0 / 3.0) * _viscosity[_qp] * _D_this_x * _grad_ux[_qp] * _normals[_qp] +
            (1.0 / 3.0) * _viscosity[_qp] * _D_this_x * _grad_ux_neighbor[_qp] * _normals[_qp]) *
           _test[_i][_qp];
      r += _epsilon * 0.5 * (_ux[_qp] - _ux_neighbor[_qp]) * (1.0 / 3.0) * _viscosity[_qp] *
           _D_this_x * _grad_test[_i][_qp] * _normals[_qp];
      r += _sigma / h_elem * (_ux[_qp] - _ux_neighbor[_qp]) * _test[_i][_qp];

      // y-dir element residuals
      r -= 0.5 *
           ((1.0 / 3.0) * _viscosity[_qp] * _D_this_y * _grad_uy[_qp] * _normals[_qp] +
            (1.0 / 3.0) * _viscosity[_qp] * _D_this_y * _grad_uy_neighbor[_qp] * _normals[_qp]) *
           _test[_i][_qp];
      r += _epsilon * 0.5 * (_uy[_qp] - _uy_neighbor[_qp]) * (1.0 / 3.0) * _viscosity[_qp] *
           _D_this_y * _grad_test[_i][_qp] * _normals[_qp];
      r += _sigma / h_elem * (_uy[_qp] - _uy_neighbor[_qp]) * _test[_i][_qp];

      // z-dir element residuals
      r -= 0.5 *
           ((1.0 / 3.0) * _viscosity[_qp] * _D_this_z * _grad_uz[_qp] * _normals[_qp] +
            (1.0 / 3.0) * _viscosity[_qp] * _D_this_z * _grad_uz_neighbor[_qp] * _normals[_qp]) *
           _test[_i][_qp];
      r += _epsilon * 0.5 * (_uz[_qp] - _uz_neighbor[_qp]) * (1.0 / 3.0) * _viscosity[_qp] *
           _D_this_z * _grad_test[_i][_qp] * _normals[_qp];
    ...
    case Moose::Neighbor:
      ...
      r += _epsilon * 0.5 * (_ux[_qp] - _ux_neighbor[_qp]) * (1.0 / 3.0) * _viscosity[_qp] *
           _D_this_x * _grad_test_neighbor[_i][_qp] * _normals[_qp];
      ...
      r += 0.5 *
           ...
           _test_neighbor[_i][_qp];
      r += _epsilon * 0.5 * (_uy[_qp] - _uy_neighbor[_qp]) * (1.0 / 3.0) * _viscosity[_qp] *
           _D_this_y * _grad_test_neighbor[_i][_qp] * _normals[_qp];
      ...
      r += 0.5 *
           ...
           _test_neighbor[_i][_qp];
      r += _epsilon * 0.5 * (_uz[_qp] - _uz_neighbor[_qp]) * (1.0 / 3.0) * _viscosity[_qp] *
           _D_this_z * _grad_test_neighbor[_i][_qp] * _normals[_qp];
...
Real
DGNSViscousVelocityDivergence::computeQpJacobian(Moose::DGJacobianType type)
{
  Real r = 0;
  ...
  if (_dir == 0)
  {
    switch (type)
    {
      ...
      case Moose::ElementElement:
        r -= 0.5 * (1.0 / 3.0) * _viscosity[_qp] * _D_this_x * _grad_phi[_j][_qp] * _normals[_qp] *
             _test[_i][_qp];
        r += _epsilon * 0.5 * _phi[_j][_qp] * (1.0 / 3.0) * _viscosity[_qp] * _D_this_x *
      ...
      case Moose::ElementNeighbor:
        r -= 0.5 * (1.0 / 3.0) * _viscosity[_qp] * _D_this_x * _grad_phi_neighbor[_j][_qp] *
  ...
  if (_dir == 1)
  {
    switch (type)
    {
      ...
      // d(R_element)/d(u)
      case Moose::ElementElement:
        r -= 0.5 * (1.0 / 3.0) * _viscosity[_qp] * _D_this_y * _grad_phi[_j][_qp] * _normals[_qp] *
             _test[_i][_qp];
        r += _epsilon * 0.5 * _phi[_j][_qp] * (1.0 / 3.0) * _viscosity[_qp] * _D_this_y *
             _grad_test[_i][_qp] * _normals[_qp];
        r += _sigma / h_elem * _phi[_j][_qp] * _test[_i][_qp];
        break;
      ...
      // d(R_element)/d(u_neighbor)
      case Moose::ElementNeighbor:
        r -= 0.5 * (1.0 / 3.0) * _viscosity[_qp] * _D_this_y * _grad_phi_neighbor[_j][_qp] *
             _normals[_qp] * _test[_i][_qp];
  ...
  if (_dir == 2)
  {
    switch (type)
    {
      ...
      // d(R_element)/d(u)
      case Moose::ElementElement:
        r -= 0.5 * (1.0 / 3.0) * _viscosity[_qp] * _D_this_z * _grad_phi[_j][_qp] * _normals[_qp] *
             _test[_i][_qp];
        r += _epsilon * 0.5 * _phi[_j][_qp] * (1.0 / 3.0) * _viscosity[_qp] * _D_this_z *
             _grad_test[_i][_qp] * _normals[_qp];
        r += _sigma / h_elem * _phi[_j][_qp] * _test[_i][_qp];
        break;
      ...
      // d(R_element)/d(u_neighbor)
      case Moose::ElementNeighbor:
        r -= 0.5 * (1.0 / 3.0) * _viscosity[_qp] * _D_this_z * _grad_phi_neighbor[_j][_qp] *
             _normals[_qp] * _test[_i][_qp];
...
Real
DGNSViscousVelocityDivergence::computeQpOffDiagJacobian(Moose::DGJacobianType type,
                                                        unsigned int jvar)
{
  Real r = 0;
  ...
  if (jvar == _viscosity_var)
  {
    switch (type)
    {
      ...
      case Moose::ElementElement:
        // x-dir element residuals
        r -= 0.5 *
             ((1.0 / 3.0) * _phi[_j][_qp] * _D_this_x * _grad_ux[_qp] * _normals[_qp] +
              (1.0 / 3.0) * _phi[_j][_qp] * _D_this_x * _grad_ux_neighbor[_qp] * _normals[_qp]) *
             _test[_i][_qp];
        r += _epsilon * 0.5 * (_ux[_qp] - _ux_neighbor[_qp]) * (1.0 / 3.0) * _phi[_j][_qp] *
             _D_this_x * _grad_test[_i][_qp] * _normals[_qp];

        // y-dir element residuals
        r -= 0.5 *
             ((1.0 / 3.0) * _phi[_j][_qp] * _D_this_y * _grad_uy[_qp] * _normals[_qp] +
              (1.0 / 3.0) * _phi[_j][_qp] * _D_this_y * _grad_uy_neighbor[_qp] * _normals[_qp]) *
             _test[_i][_qp];
        r += _epsilon * 0.5 * (_uy[_qp] - _uy_neighbor[_qp]) * (1.0 / 3.0) * _phi[_j][_qp] *
             _D_this_y * _grad_test[_i][_qp] * _normals[_qp];

        // z-dir element residuals
        r -= 0.5 *
             ((1.0 / 3.0) * _phi[_j][_qp] * _D_this_z * _grad_uz[_qp] * _normals[_qp] +
              (1.0 / 3.0) * _phi[_j][_qp] * _D_this_z * _grad_uz_neighbor[_qp] * _normals[_qp]) *
             _test[_i][_qp];
        r += _epsilon * 0.5 * (_uz[_qp] - _uz_neighbor[_qp]) * (1.0 / 3.0) * _phi[_j][_qp] *
             _D_this_z * _grad_test[_i][_qp] * _normals[_qp];
        break;
