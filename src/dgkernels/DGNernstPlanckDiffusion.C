 ...
 *    \details This file creates a discontinous Galerkin kernel for Nernst-Planck diffusion in a
 * given domain that has a variable diffusivity. The diffusivity is represented by a set of
...
InputParameters
DGNernstPlanckDiffusion::validParams()
{
  ...
  params.addRequiredCoupledVar(
      "temperature",
      "Variable for temperature of the media [NOTE: Cannot be defaulted to a single value] ");
...
Real
DGNernstPlanckDiffusion::computeQpResidual(Moose::DGResidualType type)
{
  ...
  switch (type)
  {
    case Moose::Element:
      r -= 0.5 *
           (_Diffusion *
                ...
                _grad_e_potential[_qp] * _normals[_qp] +
            _Diffusion_neighbor *
                (_porosity[_qp] * _u_neighbor[_qp] *
                 (_valence * _faraday / _gas_const / _temp_neighbor[_qp])) *
                _grad_e_potential_neighbor[_qp] * _normals[_qp]) *
           _test[_i][_qp];

      r += _epsilon * 0.5 * (_e_potential[_qp] - _e_potential_neighbor[_qp]) * _Diffusion *
           (_porosity[_qp] * _u[_qp] * (_valence * _faraday / _gas_const / _temp[_qp])) *
    ...
    case Moose::Neighbor:
      ...
      r += _epsilon * 0.5 * (_e_potential[_qp] - _e_potential_neighbor[_qp]) * _Diffusion_neighbor *
           (_porosity[_qp] * _u_neighbor[_qp] *
            (_valence * _faraday / _gas_const / _temp_neighbor[_qp])) *
           _grad_test_neighbor[_i][_qp] * _normals[_qp];

      r -= _sigma / h_elem * (_e_potential[_qp] - _e_potential_neighbor[_qp]) *
           _test_neighbor[_i][_qp];
      break;
  }
...
Real
DGNernstPlanckDiffusion::computeQpJacobian(Moose::DGJacobianType type)
{
  _Diffusion(0, 0) = _Dx[_qp];
  _Diffusion(0, 1) = 0.0;
  _Diffusion(0, 2) = 0.0;
  ...
  switch (type)
  {
    ...
    case Moose::ElementElement:
      r -= 0.5 *
           (_Diffusion *
            (_porosity[_qp] * _phi[_j][_qp] * (_valence * _faraday / _gas_const / _temp[_qp])) *
            _grad_e_potential[_qp] * _normals[_qp]) *
           _test[_i][_qp];
    ...
    case Moose::ElementNeighbor:
      r -= 0.5 *
           (_Diffusion_neighbor *
            (_porosity[_qp] * _phi_neighbor[_j][_qp] *
             (_valence * _faraday / _gas_const / _temp_neighbor[_qp])) *
            _grad_e_potential_neighbor[_qp] * _normals[_qp]) *
           _test[_i][_qp];
...
Real
DGNernstPlanckDiffusion::computeQpOffDiagJacobian(Moose::DGJacobianType type, unsigned int jvar)
{
  _Diffusion(0, 0) = _Dx[_qp];
  _Diffusion(0, 1) = 0.0;
  _Diffusion(0, 2) = 0.0;
    ...
    switch (type)
    {
      ...
      case Moose::ElementElement:
        r -= 0.5 *
             (_phi[_j][_qp] *
                  (_porosity[_qp] * _u[_qp] * (_valence * _faraday / _gas_const / _temp[_qp])) *
                  _grad_e_potential[_qp](0) * _normals[_qp](0) +
              _phi_neighbor[_j][_qp] *
                  (_porosity[_qp] * _u_neighbor[_qp] *
                   (_valence * _faraday / _gas_const / _temp_neighbor[_qp])) *
                  _grad_e_potential_neighbor[_qp](0) * _normals[_qp](0)) *
             _test[_i][_qp];
        r += _epsilon * 0.5 * (_e_potential[_qp] - _e_potential_neighbor[_qp]) * _phi[_j][_qp] *
             (_porosity[_qp] * _u[_qp] * (_valence * _faraday / _gas_const / _temp[_qp])) *
             _grad_test[_i][_qp](0) * _normals[_qp](0);
        break;
    ...
    switch (type)
    {
      ...
      case Moose::ElementElement:
        r -= 0.5 *
             (_phi[_j][_qp] *
                  (_porosity[_qp] * _u[_qp] * (_valence * _faraday / _gas_const / _temp[_qp])) *
                  _grad_e_potential[_qp](1) * _normals[_qp](1) +
              _phi_neighbor[_j][_qp] *
                  (_porosity[_qp] * _u_neighbor[_qp] *
                   (_valence * _faraday / _gas_const / _temp_neighbor[_qp])) *
                  _grad_e_potential_neighbor[_qp](1) * _normals[_qp](1)) *
             _test[_i][_qp];
        r += _epsilon * 0.5 * (_e_potential[_qp] - _e_potential_neighbor[_qp]) * _phi[_j][_qp] *
             (_porosity[_qp] * _u[_qp] * (_valence * _faraday / _gas_const / _temp[_qp])) *
             _grad_test[_i][_qp](1) * _normals[_qp](1);
        break;
    ...
    switch (type)
    {
      ...
      case Moose::ElementElement:
        r -= 0.5 *
             (_phi[_j][_qp] *
                  (_porosity[_qp] * _u[_qp] * (_valence * _faraday / _gas_const / _temp[_qp])) *
                  _grad_e_potential[_qp](2) * _normals[_qp](2) +
              _phi_neighbor[_j][_qp] *
                  (_porosity[_qp] * _u_neighbor[_qp] *
                   (_valence * _faraday / _gas_const / _temp_neighbor[_qp])) *
                  _grad_e_potential_neighbor[_qp](2) * _normals[_qp](2)) *
             _test[_i][_qp];
        r += _epsilon * 0.5 * (_e_potential[_qp] - _e_potential_neighbor[_qp]) * _phi[_j][_qp] *
             (_porosity[_qp] * _u[_qp] * (_valence * _faraday / _gas_const / _temp[_qp])) *
             _grad_test[_i][_qp](2) * _normals[_qp](2);
        break;
  ...
  if (jvar == _e_potential_id)
  {
    ...
    switch (type)
    {
      case Moose::ElementElement:
        ...
        r += _epsilon * 0.5 * _phi[_j][_qp] * _Diffusion *
             (_porosity[_qp] * _u[_qp] * (_valence * _faraday / _gas_const / _temp[_qp])) *
             _grad_test[_i][_qp] * _normals[_qp];
      ...
      case Moose::NeighborElement:
        r += 0.5 * _Diffusion *
             (_porosity[_qp] * _u[_qp] * (_valence * _faraday / _gas_const / _temp[_qp])) *
             _grad_phi[_j][_qp] * _normals[_qp] * _test_neighbor[_i][_qp];
        r += _epsilon * 0.5 * _phi[_j][_qp] * _Diffusion_neighbor *
             (_porosity[_qp] * _u_neighbor[_qp] *
              (_valence * _faraday / _gas_const / _temp_neighbor[_qp])) *
             _grad_test_neighbor[_i][_qp] * _normals[_qp];
  ...
  if (jvar == _porosity_var)
  {
    ...
    switch (type)
    {
      ...
      case Moose::ElementElement:
        r -= 0.5 *
             (_Diffusion *
                  (_phi[_j][_qp] * _u[_qp] * (_valence * _faraday / _gas_const / _temp[_qp])) *
                  _grad_e_potential[_qp] * _normals[_qp] +
              _Diffusion_neighbor *
                  (_phi[_j][_qp] * _u_neighbor[_qp] *
                   (_valence * _faraday / _gas_const / _temp_neighbor[_qp])) *
                  _grad_e_potential_neighbor[_qp] * _normals[_qp]) *
             _test[_i][_qp];
        r += _epsilon * 0.5 * (_e_potential[_qp] - _e_potential_neighbor[_qp]) * _Diffusion *
  ...
  if (jvar == _temp_id)
  {
    ...
    switch (type)
    {
      ...
      case Moose::ElementElement:
        r -= 0.5 *
             (_Diffusion * (-1.0 / _temp[_qp]) * _phi[_j][_qp] *
              (_porosity[_qp] * _u[_qp] * (_valence * _faraday / _gas_const / _temp[_qp])) *
              _grad_e_potential[_qp] * _normals[_qp]) *
             _test[_i][_qp];
        r += _epsilon * 0.5 * (_e_potential[_qp] - _e_potential_neighbor[_qp]) * _Diffusion *
      ...
      case Moose::ElementNeighbor:
        r -= 0.5 *
             (_Diffusion_neighbor * (-1.0 / _temp_neighbor[_qp]) * _phi_neighbor[_j][_qp] *
              (_porosity[_qp] * _u_neighbor[_qp] *
               (_valence * _faraday / _gas_const / _temp_neighbor[_qp])) *
              _grad_e_potential_neighbor[_qp] * _normals[_qp]) *
             _test[_i][_qp];
