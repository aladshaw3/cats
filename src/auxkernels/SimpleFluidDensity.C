...
InputParameters SimpleFluidDensity::validParams()
{
    InputParameters params = SimpleFluidPropertiesBase::validParams();
    params.addParam< std::string >("output_volume_unit","L","Volume units for fluid density on output");
...
Real SimpleFluidDensity::computeValue()
{
    ...
    rho = 1/SimpleFluidPropertiesBase::volume_conversion(1/rho, _rho_volume_unit, _output_volume_unit);
    return rho;
}
