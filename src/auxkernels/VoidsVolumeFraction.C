 ...
 *                Ref:  Hilbert, D. and Cohn-Vossen, S. Geometry and the Imagination. New York:
 * Chelsea, pp. 45-53, 1999.
 *
 *            Minimum allowed voids fraction = 0.26
 *                Theoretical minimum based on very tight packing of spheres
 *                Ref:  Steinhaus, H. Mathematical Snapshots, 3rd ed. New York: Dover, pp. 202-203,
 * 1999. Ref:  Wells, D. The Penguin Dictionary of Curious and Interesting Numbers. Middlesex,
 * England: Penguin Books, p. 29, 1986.
 *
...
InputParameters
VoidsVolumeFraction::validParams()
{
  InputParameters params = AuxKernel::validParams();
  params.addParam<Real>(
      "particle_diameter", 1, "Average diameter of the particles for ratio calculation");
  ...
  params.addParam<bool>("override_limits",
                        false,
                        "If true, then ratio is allowed to go beyond physical limits. If false, "
                        "then ratio is bounded to theoretical max and min ");
  return params;
}
    ...
    _OverrideLimits(getParam<bool>("override_limits"))
{
  ...
  if (_OverrideLimits == false)
  {
    if (_void_fraction > 0.66)
    {
      std::cout << "\n================================= WARNING "
                   "===========================================\n";
      std::cout << "  Calculated void fraction (" << _void_fraction
                << ") is above 0.66 theoretical limit\n";
      std::cout << "     Value is being automatically reset to 0.66\n";
      std::cout
          << "     To avoid reset, you can set 'override_limits' to 'true' in your input file\n";
    ...
    if (_void_fraction < 0.26)
    {
      std::cout << "\n================================= WARNING "
                   "===========================================\n";
      std::cout << "  Calculated void fraction (" << _void_fraction
                << ") is below 0.26 theoretical limit\n";
...
Real
VoidsVolumeFraction::computeValue()
{
