...
InputParameters AuxElectrolyteCurrent::validParams()
{
...
Real AuxElectrolyteCurrent::sum_ion_terms()
{
    Real sum = 0.0;
    for (unsigned int i = 0; i<_ion_conc.size(); ++i)
    {
...
Real AuxElectrolyteCurrent::effective_ionic_conductivity()
{
...
Real AuxElectrolyteCurrent::computeValue()
{
    return -effective_ionic_conductivity()*(_norm_vec*_e_potential_grad[_qp]) - _faraday*_porosity[_qp]*sum_ion_gradient_terms();
}
