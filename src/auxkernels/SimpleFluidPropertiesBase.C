...
InputParameters
SimpleFluidPropertiesBase::validParams()
{
  ...
  params.addParam<bool>(
      "use_pressure_units",
      true,
      "If true, the 'pressure_unit' is used for pressure, else the base SI units are used");
  ...
  params.addCoupledVar(
      "characteristic_length", 1.0, "Name of the length variable (e.g., hydraulic diameter)");
  params.addParam<std::string>("char_length_unit", "m", "Length units for characteristic length");

  params.addParam<Real>(
      "ref_diffusivity",
      2.296E-5,
      "Reference diffusivity value (e.g., molecular fluid-phase diffusion constant)");
  ...
  params.addParam<Real>(
      ...
      0.5,
      "Factor applied to pore diffusivity to estimate effective diffusion: Range (0,2)");
  ...
  params.addParam<Real>(
      "viscosity_pre_exponential", 0.02939, "Pre-exponential factor for viscosity calculation");
  ...
  params.addParam<Real>(
      "viscosity_param_B", 507.88, "B parameter in the viscosity calculation (K)");
  ...
  params.addParam<std::string>(
      "ionic_strength_volume_unit", "mL", "Volume units for ionic strength");
  params.addParam<Real>("ionic_strength_param_A",
                        0.0062,
                        "A parameter in the ionic strength correction calculation (L/mol)^0.5");
  params.addParam<Real>("ionic_strength_param_B",
                        0.0793,
                        "B parameter in the ionic strength correction calculation (L/mol)^1");
  params.addParam<Real>("ionic_strength_param_C",
                        0.0080,
                        "C parameter in the ionic strength correction calculation (L/mol)^2");

  params.addParam<Real>(
      "density_param_A", -2.9335E-6, "A parameter in the density calculation (K)^-2");
  params.addParam<Real>(
      "density_param_B", 0.001529811, "B parameter in the density calculation (K)^-1");
...
void
SimpleFluidPropertiesBase::unsupported_conversion(std::string from, std::string to)
{
  std::cout << "SimpleGasPropertiesBase has encountered an error...\n";
  std::cout << "\t from: " << from << ", to: " << to
            << " is not a currently supported unit conversion...\n";
  moose::internal::mooseErrorRaw("Given unit is unsupported! Forced Exit...");
}

Real
SimpleFluidPropertiesBase::length_conversion(Real value, std::string from, std::string to)
{
  Real new_value = 0;
...
Real
SimpleFluidPropertiesBase::time_conversion(Real value, std::string from, std::string to)
{
  Real new_value = 0;
...
Real
SimpleFluidPropertiesBase::mass_conversion(Real value, std::string from, std::string to)
{
  Real new_value = 0;
...
Real
SimpleFluidPropertiesBase::energy_conversion(Real value, std::string from, std::string to)
{
  Real new_value = 0;
...
Real
SimpleFluidPropertiesBase::pressure_conversion(Real value, std::string from, std::string to)
{
  Real new_value = 0;
...
Real
SimpleFluidPropertiesBase::volume_conversion(Real value, std::string from, std::string to)
{
  Real new_value = 0;
...
Real
SimpleFluidPropertiesBase::fluid_viscosity(Real temperature)
{
  return _mu_pre_exp * std::exp(_mu_B / (temperature - _mu_C));
}

Real
SimpleFluidPropertiesBase::fluid_viscosity_with_ionic_strength_correction(Real temperature,
                                                                          Real ionic_strength)
{
  if (ionic_strength > 0.0)
    ...
    ionic_strength = 0;
  Real coeff = 1.0 + _mu_I_A * std::sqrt(ionic_strength) + _mu_I_B * ionic_strength +
               _mu_I_C * ionic_strength * ionic_strength;
  return fluid_viscosity(temperature) * coeff;
}

Real
SimpleFluidPropertiesBase::fluid_density(Real temperature, Real pressure)
{
...
Real
SimpleFluidPropertiesBase::molecular_diffusion(Real temperature)
{
  return _ref_diffusivity * std::exp(-1991.805 * ((1 / temperature) - (1 / _ref_diff_temp)));
}

Real
SimpleFluidPropertiesBase::effective_molecular_diffusion(Real temperature, Real porosity)
{
  if (porosity > 1 || porosity < 0)
    moose::internal::mooseErrorRaw(
        "Variable for 'macro_porosity' must be strictly < 1 and > 0 [0 < eps < 1]");
  return molecular_diffusion(temperature) * std::pow(porosity, _eff_diff_factor);
}

Real
SimpleFluidPropertiesBase::dispersion(Real temperature)
{
...
Real
SimpleFluidPropertiesBase::effective_dispersion(Real temperature, Real porosity)
{
  if (porosity > 1 || porosity < 0)
    moose::internal::mooseErrorRaw(
        "Variable for 'macro_porosity' must be strictly < 1 and > 0 [0 < eps < 1]");
...
Real
SimpleFluidPropertiesBase::velocity_magnitude(Real ux, Real uy, Real uz)
{
  return std::sqrt(ux * ux + uy * uy + uz * uz);
}

Real
SimpleFluidPropertiesBase::reynolds_number()
{
...
Real
SimpleFluidPropertiesBase::schmidt_number()
{
...
Real
SimpleFluidPropertiesBase::computeValue()
{
