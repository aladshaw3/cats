...
InputParameters
SimpleFluidDispersion::validParams()
{
  ...
  params.addParam<bool>(
      "include_dispersivity_correction",
      true,
      "If true, then the calculation includes a correction based on the 'dispersivity' parameter");
  params.addParam<bool>("include_porosity_correction",
                        true,
                        "If true, then the calculation includes a correction based on the "
                        "'effective_diffusivity_factor' parameter");
...
Real
SimpleFluidDispersion::computeValue()
{
  Real D = 0;
  ...
  else
  {
    if (_includePorosityCorrection == true)
    {
      D = SimpleFluidPropertiesBase::effective_molecular_diffusion(_temperature[_qp],
                                                                   _macro_pore[_qp]);
    }
    else
    {
