 ...
 *    \brief AuxKernel kernel to compute store information associated with calculation of gas
 * properties \details This file is responsible for calculating storing and calculating a number of
 * gas properties that will be used by other auxkernels that inherit from this base kernel.
 *
 *
...
GasPropertiesBase::validParams()
{
  ...
  params.addRequiredParam<std::vector<Real>>("sutherland_temp",
                                             "List of Sutherland's reference temperatures (K)");
  params.addRequiredParam<std::vector<Real>>("sutherland_const",
                                             "List of Sutherland's constants (K)");
  params.addRequiredParam<std::vector<Real>>("sutherland_vis",
                                             "List of Sutherland's reference viscosities (g/cm/s)");
  ...
  params.addParam<bool>("is_ideal_gas",
                        true,
                        "If true, then densities used for kinetic theory and pressure drop are "
                        "calculated from given pressure assuming ideal gas law. If false, "
                        "densities are calculated solely from the given gas concentrations.");
  return params;
}
...
void
GasPropertiesBase::prepareEgret()
{
  int success = 0;
  _total_conc = 0.0;
...
void
GasPropertiesBase::calculateAllProperties()
{
  ...
  int success = set_variables(_press[_qp] / 1000.0,
                              _temp[_qp],
                              vel_mag * 100.0,
                              _char_len[_qp] * 100.0,
                              _mole_frac,
...
Real
GasPropertiesBase::computeValue()
{
  prepareEgret();
