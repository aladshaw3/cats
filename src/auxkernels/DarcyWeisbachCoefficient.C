 ...
 *    \brief AuxKernel kernel base to calculate the coefficient to be used in the Darcy-Weisbach relationship
 *    \details This file is responsible for calculating the Darcy-Weisbach coefficient value
 *              to be used with the VariableLaplacian kernel to resolve pressure gradients
 *              in a fluid conduit of given hydraulic diameter. Those pressure gradients
 *              are then coupled with the velocity vector for superficial velocity in the media.
 *
 ...
 */
 ...
 InputParameters DarcyWeisbachCoefficient::validParams()
 {
     ...
     params.addCoupledVar("hydraulic_diameter",0.01,"Approximate hydraulic diameter of the domain (can vary spatially)");
     return params;
 }
 ...
 Real DarcyWeisbachCoefficient::computeValue()
 {
