...
InputParameters
AuxRatio::validParams()
{
  InputParameters params = AuxKernel::validParams();
  params.addRequiredCoupledVar("numerator_vars", "List of variables for the numerator (top)");
  params.addRequiredCoupledVar("denominator_vars",
                               "List of variables for the denominator (bottom)");

  return params;
}

AuxRatio::AuxRatio(const InputParameters & parameters) : AuxKernel(parameters)
{
