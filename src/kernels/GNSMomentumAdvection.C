...
InputParameters
GNSMomentumAdvection::validParams()
{
...
Real
GNSMomentumAdvection::computeQpResidual()
{
  _velocity(0) = _ux[_qp];
  _velocity(1) = _uy[_qp];
  _velocity(2) = _uz[_qp];
...
Real
GNSMomentumAdvection::computeQpJacobian()
{
  _velocity(0) = _ux[_qp];
  _velocity(1) = _uy[_qp];
  _velocity(2) = _uz[_qp];

  return GAdvection::computeQpJacobian() * _density[_qp] +
         (-_u[_qp] * (_phi[_j][_qp] * _grad_test[_i][_qp](_dir)) * _density[_qp]);
}

Real
GNSMomentumAdvection::computeQpOffDiagJacobian(unsigned int jvar)
{
  _velocity(0) = _ux[_qp];
  _velocity(1) = _uy[_qp];
  _velocity(2) = _uz[_qp];
