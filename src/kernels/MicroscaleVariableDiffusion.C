...
InputParameters
MicroscaleVariableDiffusion::validParams()
{
...
Real
MicroscaleVariableDiffusion::computeQpResidual()
{
...
Real
MicroscaleVariableDiffusion::computeQpJacobian()
{
  _current_diff = _current_diffusion[_qp];
  _upper_diff = _upper_diffusion[_qp];
  _lower_diff = _lower_diffusion[_qp];
...
Real
MicroscaleVariableDiffusion::computeQpOffDiagJacobian(unsigned int jvar)
{
  _current_diff = _current_diffusion[_qp];
  _upper_diff = _upper_diffusion[_qp];
  _lower_diff = _lower_diffusion[_qp];
  ...
  if (jvar == _current_diff_var)
  {
    return _test[_i][_qp] * ((_rd_lp1 / _dr / _dr / 2.0)) * _phi[_j][_qp] *
           ...
           _test[_i][_qp] * ((_rd_lm1 / _dr / _dr / 2.0)) * _phi[_j][_qp] *
