 ...
 *  \brief Kernel for creating an Arrhenius reaction coupled with temperature added to an energy
 * phase residual \details This file creates a standard MOOSE kernel for the coupling a set of
 * non-linear variables to create an Arrhenius reaction acting on an energy balance. This kernel has
...
InputParameters
ArrheniusReactionEnergyTransfer::validParams()
{
...
Real
ArrheniusReactionEnergyTransfer::computeQpResidual()
{
...
Real
ArrheniusReactionEnergyTransfer::computeQpJacobian()
{
  return 0.0;
}

Real
ArrheniusReactionEnergyTransfer::computeQpOffDiagJacobian(unsigned int jvar)
{
