 ...
 *DGPoreConcAdvection for the discontinous Galerkin formulation of advection in MOOSE. In order to
 *complete the DG formulation of the advective physics, this kernel must be utilized with every
 *variable that also uses the DGPoreConcAdvection kernel.
 *
...
InputParameters
GPoreConcAdvection::validParams()
{
...
Real
GPoreConcAdvection::computeQpResidual()
{
  _velocity(0) = _ux[_qp];
  _velocity(1) = _uy[_qp];
  _velocity(2) = _uz[_qp];
...
Real
GPoreConcAdvection::computeQpJacobian()
{
  _velocity(0) = _ux[_qp];
  _velocity(1) = _uy[_qp];
  _velocity(2) = _uz[_qp];
...
Real
GPoreConcAdvection::computeQpOffDiagJacobian(unsigned int jvar)
{
  _velocity(0) = _ux[_qp];
  _velocity(1) = _uy[_qp];
  _velocity(2) = _uz[_qp];
