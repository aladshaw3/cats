...
InputParameters
VariableCoupledCoeffTimeDerivative::validParams()
{
  InputParameters params = CoupledCoeffTimeDerivative::validParams();
  params.addRequiredCoupledVar(
      "var_time_coeff",
      "Variable for the coupled time derivative (often the bulk density for a phase change)");
  return params;
}

VariableCoupledCoeffTimeDerivative::VariableCoupledCoeffTimeDerivative(
...
Real
VariableCoupledCoeffTimeDerivative::computeQpResidual()
{
  if (_gaining == true)
...
Real
VariableCoupledCoeffTimeDerivative::computeQpJacobian()
{
  return 0.0;
}

Real
VariableCoupledCoeffTimeDerivative::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (_gaining == true)
    _time_coef = -_coeff[_qp];
  else
