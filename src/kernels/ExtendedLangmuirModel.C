 ...
 *	\brief Standard kernel for coupling a vector non-linear variables via an extended langmuir
 *function \details This file creates a standard MOOSE kernel for the coupling of a vector
 *non-linear variables together via an extended langmuir forcing function, i.e., variable = b_i *
 *K_i * coupled_variable_i / 1 + sum(j, K_j * coupled_variable_j). In this kernel, the langmuir
 *coefficients {K_i} are calculated as a function of temperature using the van't Hoff expression:
 *ln(K_i) = -dH_i/(R*T) + dS_i/R
 *
...
InputParameters
ExtendedLangmuirModel::validParams()
{
  InputParameters params = ExtendedLangmuirFunction::validParams();
  params.addRequiredParam<std::vector<Real>>("enthalpies",
                                             "Enthalpies for the Langmuir Coefficients (J/mol)");
  params.addRequiredParam<std::vector<Real>>("entropies",
                                             "Entropies for the Langmuir Coefficients (J/K/mol)");
    ...
    _coupled_var_temp(coupled("coupled_temp"))
{
  ...
  {
    moose::internal::mooseErrorRaw("User is required to provide list of variables of the same "
                                   "length as list of enthalpy coefficients.");
  }

  if (_coupled.size() != _entropies.size())
  {
    moose::internal::mooseErrorRaw("User is required to provide list of variables of the same "
                                   "length as list of entropy coefficients.");
  }
...
Real
ExtendedLangmuirModel::computeExtLangmuirTempJacobi()
{
  ...
  return _maxcap * _langmuir_coef[_lang_index] * _coupled_i[_qp] * _phi[_j][_qp] *
         (1.0 / Rstd / _coupled_temp[_qp] / _coupled_temp[_qp]) * numerator / denom;
}

Real
ExtendedLangmuirModel::computeQpResidual()
{
...
Real
ExtendedLangmuirModel::computeQpJacobian()
{
  return ExtendedLangmuirFunction::computeQpJacobian();
}

Real
ExtendedLangmuirModel::computeQpOffDiagJacobian(unsigned int jvar)
{
  computeAllLangmuirCoeffs();
