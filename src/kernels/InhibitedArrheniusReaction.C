...
InputParameters
InhibitedArrheniusReaction::validParams()
{
  InputParameters params = ArrheniusReaction::validParams();
  params.addRequiredCoupledVar("forward_inhibition",
                               "Name of the coupled forward inhibition variable (-)");
  params.addCoupledVar(
      "reverse_inhibition", 1.0, "Name of the coupled reverse inhibition variable (-)");
  return params;
}
...
void
InhibitedArrheniusReaction::calculateInhibitedRateConstants()
{
...
Real
InhibitedArrheniusReaction::computeQpResidual()
{
...
Real
InhibitedArrheniusReaction::computeQpJacobian()
{
  calculateInhibitedRateConstants();
...
Real
InhibitedArrheniusReaction::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _temp_var)
  {
    calculateInhibitedRateConstants();
    ...
    jac = -_scale * _forward_rate * react_prod * _test[_i][_qp] *
          ((_act_energy_for / Rstd / _temp[_qp] / _temp[_qp]) +
           _beta_for * std::pow(_temp[_qp], -1.0)) *
          _phi[_j][_qp];
    jac += _scale * _reverse_rate * prod_prod * _test[_i][_qp] *
           ((_act_energy_rev / Rstd / _temp[_qp] / _temp[_qp]) +
            _beta_rev * std::pow(_temp[_qp], -1.0)) *
           _phi[_j][_qp];
    return jac;
  }
  else if (jvar == _Rf_var)
  {
    ...
    jac = _scale * _forward_rate * react_prod * _test[_i][_qp] * _phi[_j][_qp] /
          _forward_inhibition[_qp];
  ...
  else if (jvar == _Rr_var)
  {
    ...
    jac = -_scale * _reverse_rate * prod_prod * _test[_i][_qp] * _phi[_j][_qp] /
          _reverse_inhibition[_qp];
