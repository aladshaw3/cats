/*!
 *  \file MicroscaleDiffusionInnerBC.h
 *    \brief Custom kernel for a no flux diffusion BC in a fictious microscale.
...
MicroscaleDiffusionInnerBC::MicroscaleDiffusionInnerBC(const InputParameters & parameters)
: Kernel(parameters),
...
_upper_var(coupled("upper_neighbor"))
{
    ...
    if (_lower_node > (int)(_total_nodes-1))
    {
        std::cout << "lower node = " << _lower_node << std::endl;
        std::cout << "total nodes = " << _total_nodes << std::endl;
...
Real MicroscaleDiffusionInnerBC::computeQpResidual()
{
    calculateFluxes();
    return _test[_i][_qp]*(_flux_upper+_flux_lower)*(_u[_qp] - _upper_neighbor[_qp]);
}

Real MicroscaleDiffusionInnerBC::computeQpJacobian()
{
    calculateFluxes();
