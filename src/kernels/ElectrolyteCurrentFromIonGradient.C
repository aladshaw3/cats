 ...
 *In the case of anisotopic diffusion, the diffusion coefficent the user provides should correspond
 *to the direction of the electrolyte current this kernel acts on.
 *
...
InputParameters
ElectrolyteCurrentFromIonGradient::validParams()
{
...
ElectrolyteCurrentFromIonGradient::ElectrolyteCurrentFromIonGradient(
...
Real
ElectrolyteCurrentFromIonGradient::sum_ion_gradient_terms()
{
  Real sum = 0.0;
  for (unsigned int i = 0; i < _ion_conc_grad.size(); ++i)
  {
...
Real
ElectrolyteCurrentFromIonGradient::computeQpResidual()
{
  return _test[_i][_qp] * _faraday * _porosity[_qp] * sum_ion_gradient_terms();
}

Real
ElectrolyteCurrentFromIonGradient::computeQpJacobian()
{
  return 0.0;
}

Real
ElectrolyteCurrentFromIonGradient::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  for (unsigned int i = 0; i < _ion_conc_grad.size(); ++i)
  {
    if (jvar == _ion_conc_vars[i])
      offjac = _test[_i][_qp] * _faraday * _porosity[_qp] * _valence[i] * (*_diffusion[i])[_qp] *
               (_norm_vec * _grad_phi[_j][_qp]);
    break;
    if (jvar == _diffusion_vars[i])
      offjac = _test[_i][_qp] * _faraday * _porosity[_qp] * _valence[i] * _phi[_j][_qp] *
               (_norm_vec * (*_ion_conc_grad[i])[_qp]);
    break;
  }
