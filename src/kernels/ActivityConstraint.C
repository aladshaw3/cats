 ...
 *  \brief Kernel for calculating activity of a chemical species based on activity coefficient and
 * concentration \details This file creates a kernel for the coupling a concentration of a species
 * (C) with an activity coefficient (gamma) through a reference concentration (Cref) in order to
 * establish the activity of that species in solution or on a surface. The activity coefficient
 * (gamma) should be a unitless number. The Cref concentration is generally either total
 * concentration or a reference concentration for a given phase (e.g., in electrolyte chemistry,
 * this is generally taken to be 1 M). The concentration for the species must have same units as
 * Cref such that the calculated activity (a) is unitless.
 *
...
InputParameters
ActivityConstraint::validParams()
{
...
Real
ActivityConstraint::computeQpResidual()
{
  return -_test[_i][_qp] * _gamma[_qp] * _conc[_qp] / _ref_conc[_qp];
}

Real
ActivityConstraint::computeQpJacobian()
{
  return 0.0;
}

Real
ActivityConstraint::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  {
    return _test[_i][_qp] * _gamma[_qp] * _conc[_qp] / _ref_conc[_qp] / _ref_conc[_qp] *
           _phi[_j][_qp];
  }
