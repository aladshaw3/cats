...
InputParameters
GElectrolyteOhmicHeating::validParams()
{
...
Real
GElectrolyteOhmicHeating::sum_ion_terms()
{
  Real sum = 0.0;
  for (unsigned int i = 0; i < _ion_conc.size(); ++i)
  {
...
Real
GElectrolyteOhmicHeating::effective_ionic_conductivity()
{
...
Real
GElectrolyteOhmicHeating::computeQpResidual()
{
  return -_test[_i][_qp] * effective_ionic_conductivity() *
         (_e_potential_grad[_qp] * _e_potential_grad[_qp]);
}

Real
GElectrolyteOhmicHeating::computeQpJacobian()
{
  return 0.0;
}

Real
GElectrolyteOhmicHeating::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _porosity_var)
  {
    return -_test[_i][_qp] * (_faraday * _faraday / _gas_const / _temp[_qp]) * _phi[_j][_qp] *
           sum_ion_terms() * (_e_potential_grad[_qp] * _e_potential_grad[_qp]);
  }
  if (jvar == _temp_var)
  {
    return -_test[_i][_qp] * effective_ionic_conductivity() *
           (_e_potential_grad[_qp] * _e_potential_grad[_qp]) * (-1.0 / _temp[_qp]) * _phi[_j][_qp];
  }
