...
MicroscaleDiffusionOuterBC::MicroscaleDiffusionOuterBC(const InputParameters & parameters)
: Kernel(parameters),
_diff_const(getParam<Real>("diffusion_const")),
_trans_const(getParam<Real>("transfer_const")),
...
_macro_variable(coupledValue("macro_variable")),
_macro_var(coupled("macro_variable")),
...
_lower_var(coupled("lower_neighbor"))
{
    ...
    {
        moose::internal::mooseErrorRaw("This Node id given is beyond limits! ( 0 <= node_id < _total_nodes )");
    }
    _upper_node = _this_node + 1;
    if (_upper_node < 0 || _upper_node > _total_nodes)
    {
        moose::internal::mooseErrorRaw("Upper Node id calculated is beyond limits! ( 0 <= node_id < _total_nodes )");
    }
    _lower_node = _this_node - 1;
    if (_lower_node < 0 || _lower_node > (int)(_total_nodes-1))
    {
        moose::internal::mooseErrorRaw("Lower Node id calculated is beyond limits! ( 0 <= node_id < _total_nodes )");
    }
    if (_total_nodes < 2)
    {
...
Real MicroscaleDiffusionOuterBC::computeQpResidual()
{
    calculateFluxes();
    return _test[_i][_qp]*_flux_upper*(_u[_qp] - _macro_variable[_qp]) + _test[_i][_qp]*_flux_lower*(_u[_qp] - _lower_neighbor[_qp]);
}

Real MicroscaleDiffusionOuterBC::computeQpJacobian()
{
    calculateFluxes();
    return _test[_i][_qp]*_flux_upper*_phi[_j][_qp] + _test[_i][_qp]*_flux_lower*_phi[_j][_qp];
}
Real MicroscaleDiffusionOuterBC::computeQpOffDiagJacobian(unsigned int jvar)
{
    calculateFluxes();
    if (jvar == _macro_var)
    {
