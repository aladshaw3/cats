...
InputParameters
MicroscaleDiffusionOuterBC::validParams()
{
  InputParameters params = Kernel::validParams();
  params.addParam<Real>("diffusion_const", 1.0, "[Global] Diffusion constant in the microscale");
  params.addParam<Real>(
      "transfer_const", 1.0, "Mass transfer constant between micro- and macro-scales");
  ...
  params.addRequiredCoupledVar("macro_variable",
                               "Variable for macroscale problem (i.e., actual mesh)");
...
void
MicroscaleDiffusionOuterBC::calculateFluxes()
{
  ...
  _flux_lower =
      ((_rd_lm1 * _avg_lower_diff) / _dr / _dr) + ((_rd_lp1 * _avg_upper_diff) / _dr / _dr);
}

Real
MicroscaleDiffusionOuterBC::computeQpResidual()
{
  calculateFluxes();
  return _test[_i][_qp] * _flux_upper * (_u[_qp] - _macro_variable[_qp]) +
         _test[_i][_qp] * _flux_lower * (_u[_qp] - _lower_neighbor[_qp]);
}

Real
MicroscaleDiffusionOuterBC::computeQpJacobian()
{
  calculateFluxes();
...
Real
MicroscaleDiffusionOuterBC::computeQpOffDiagJacobian(unsigned int jvar)
{
  calculateFluxes();
  if (jvar == _macro_var)
  {
