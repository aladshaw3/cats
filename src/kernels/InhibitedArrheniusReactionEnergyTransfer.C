 ...
 *  \brief Kernel for creating an Arrhenius reaction coupled with inhibition added to an energy
 * phase residual \details This file creates a standard MOOSE kernel for the coupling a set of
 * non-linear variables to create an inhibited Arrhenius reaction acting on an energy balance. This
 * kernel has a list of reactants and a list of products, with corresponding lists for
 * stoichiometric coefficients. The residual for this kernel is as follows Res = - a*kf/Rf*prod(C_i,
 * v_i) + a*kr/Rr*prod(C_j, v_j) where a = scaling parameter, kf = forward rate, kr = reverse rate,
...
InputParameters
InhibitedArrheniusReactionEnergyTransfer::validParams()
{
...
InhibitedArrheniusReactionEnergyTransfer::InhibitedArrheniusReactionEnergyTransfer(
...
Real
InhibitedArrheniusReactionEnergyTransfer::computeQpResidual()
{
...
Real
InhibitedArrheniusReactionEnergyTransfer::computeQpJacobian()
{
  return 0.0;
}

Real
InhibitedArrheniusReactionEnergyTransfer::computeQpOffDiagJacobian(unsigned int jvar)
{
