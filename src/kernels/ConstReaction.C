...
InputParameters
ConstReaction::validParams()
{
  InputParameters params = Kernel::validParams();
  params.addRequiredParam<std::vector<Real>>("reactant_stoich",
                                             "List of stoichiometry for reactants");
  params.addRequiredParam<std::vector<Real>>("product_stoich",
                                             "List of stoichiometry for products");
    ...
    _main_var(coupled("this_variable"))
{
  ...
  {
    moose::internal::mooseErrorRaw("User is required to provide list of reactant variables of the "
                                   "same length as list of reactant stoichiometry.");
  }

  if (_products.size() != _prod_stoich.size())
  {
    moose::internal::mooseErrorRaw("User is required to provide list of product variables of the "
                                   "same length as list of product stoichiometry.");
  }
...
Real
ConstReaction::computeQpResidual()
{
  Real res = 0.0;
  Real react_prod = 1.0;
  Real prod_prod = 1.0;
  ...
  res = -_scale * _forward_rate * react_prod * _test[_i][_qp] +
        _scale * _reverse_rate * prod_prod * _test[_i][_qp];
  return res;
}

Real
ConstReaction::computeQpJacobian()
{
  Real jac = 0.0;
  ...
  if (inReactList == true)
  {
    ...
    jac += -react_prod * _scale * _forward_rate * _react_stoich[indexReact] *
           std::pow((*_reactants[indexReact])[_qp], _react_stoich[indexReact] - 1.0) *
  ...
  if (inProdList == true)
  {
    ...
    jac += prod_prod * _scale * _reverse_rate * _prod_stoich[indexProd] *
           std::pow((*_products[indexProd])[_qp], _prod_stoich[indexProd] - 1.0) * _test[_i][_qp] *
           _phi[_j][_qp];
  }
  return jac;
}
...
Real
ConstReaction::computeQpOffDiagJacobian(unsigned int jvar)
{
  bool exists = false;
  for (unsigned int i = 0; i < _reactants.size(); ++i)
  {
  ...
  if (ri == -1)
    ...
    react_prod = react_prod * _react_stoich[ri] *
                 std::pow((*_reactants[ri])[_qp], _react_stoich[ri] - 1.0) * _test[_i][_qp] *
                 _phi[_j][_qp];
  for (unsigned int i = 0; i < _products.size(); ++i)
  {
  ...
  if (pi == -1)
    ...
    prod_prod = prod_prod * _prod_stoich[pi] *
                std::pow((*_products[pi])[_qp], _prod_stoich[pi] - 1.0) * _test[_i][_qp] *
                _phi[_j][_qp];
