...
InputParameters
ElectrolyteIonConductivity::validParams()
{
...
Real
ElectrolyteIonConductivity::sum_ion_gradient_terms()
{
  Real sum = 0.0;
  for (unsigned int i = 0; i < _ion_conc_grad.size(); ++i)
  {
    sum = sum +
          _valence[i] * (*_diffusion[i])[_qp] * (_grad_test[_i][_qp] * (*_ion_conc_grad[i])[_qp]);
  }
  return sum;
}

Real
ElectrolyteIonConductivity::computeQpResidual()
{
  return _faraday * _porosity[_qp] * sum_ion_gradient_terms();
}

Real
ElectrolyteIonConductivity::computeQpJacobian()
{
  return 0.0;
}

Real
ElectrolyteIonConductivity::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  for (unsigned int i = 0; i < _ion_conc_grad.size(); ++i)
  {
    if (jvar == _ion_conc_vars[i] && _tight == true)
      offjac = _faraday * _porosity[_qp] * _valence[i] * (*_diffusion[i])[_qp] *
               (_grad_test[_i][_qp] * _grad_phi[_j][_qp]);
    break;
    if (jvar == _diffusion_vars[i])
      offjac = _faraday * _porosity[_qp] * _valence[i] * _phi[_j][_qp] *
               (_grad_test[_i][_qp] * (*_ion_conc_grad[i])[_qp]);
    break;
  }
