...
InputParameters PairedLangmuirInhibition::validParams()
{
...
void PairedLangmuirInhibition::computeAllLangmuirCoeffs()
{
    LangmuirInhibition::computeAllLangmuirCoeffs();
    for (int n=0; n<_binary_coef.size(); n++)
    {
        _binary_coef[n] = _binary_pre_exp[n] * std::pow(_temp[_qp], _binary_beta[n]) * std::exp(-_binary_act_energy[n]/Rstd/_temp[_qp]);
    }
}

Real PairedLangmuirInhibition::computePairedLangmuirTerm(int n)
{
    Real val = 0.0;
...
Real PairedLangmuirInhibition::computePairedLangmuirConcJacobi(int jvar)
{
    Real val = 0.0;
    for (int n=0; n<_binary_coef.size(); n++)
    {
...
Real PairedLangmuirInhibition::computePairedLangmuirTempJacobiTerm(int n)
{
    Real val = 0.0;
    if ((*_coupled_i[n])[_qp] > 0.0 && (*_coupled_j[n])[_qp] > 0.0)
        val = _binary_coef[n] * (*_coupled_i[n])[_qp] * (*_coupled_j[n])[_qp] * ((_binary_act_energy[n]/Rstd/_temp[_qp]/_temp[_qp]) + (_binary_beta[n]/_temp[_qp])) * _phi[_j][_qp];
    return val;
}

Real PairedLangmuirInhibition::computePairedLangmuirTempJacobi()
{
    Real val = 0.0;
    for (unsigned int n = 0; n<_coupled_i.size(); ++n)
    {
...
Real PairedLangmuirInhibition::computeQpResidual()
{
...
Real PairedLangmuirInhibition::computeQpJacobian()
{
    return 0.0;
}
