 ...
 *function with variable site density \details This file creates a standard MOOSE kernel for the
 *coupling of a vector non-linear variables together via an extended langmuir forcing function while
 *including a coupling for the site density term. Utilization of this kernel will allow users to
 *specify a function or set of residuals to solve for the site density and couple that site density
 *to this function that calculates the Langmuir capacity in terms of that site density. This is
 *particularly useful to adding the effects of aging to the model.
 *
...
InputParameters
VarSiteDensityExtLangModel::validParams()
{
  InputParameters params = ExtendedLangmuirModel::validParams();
  params.addRequiredCoupledVar("coupled_site_density",
                               "Name of the coupled site density variable (mol/L)");
  return params;
}
...
Real
VarSiteDensityExtLangModel::computeExtLangmuirSiteJacobi()
{
  double sum = 0.0;
  for (unsigned int i = 0; i < _coupled.size(); ++i)
  {
...
Real
VarSiteDensityExtLangModel::computeQpResidual()
{
...
Real
VarSiteDensityExtLangModel::computeQpJacobian()
{
  _maxcap = _coupled_site_density[_qp];
...
Real
VarSiteDensityExtLangModel::computeQpOffDiagJacobian(unsigned int jvar)
{
