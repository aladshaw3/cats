...
void EquilibriumReaction::calculateEquilibriumConstant()
{
    _forward_rate = std::exp( lnKo(_enthalpy, _entropy, _temp[_qp]) );
}

Real EquilibriumReaction::computeQpResidual()
{
    calculateEquilibriumConstant();
    return ConstReaction::computeQpResidual();
}

Real EquilibriumReaction::computeQpJacobian()
{
    calculateEquilibriumConstant();
    return ConstReaction::computeQpJacobian();
}

Real EquilibriumReaction::computeQpOffDiagJacobian(unsigned int jvar)
{
    if (jvar == _temp_var)
    {
        calculateEquilibriumConstant();
        ...
        return -_scale*react_prod*_forward_rate * (_enthalpy/Rstd/_temp[_qp]/_temp[_qp]) * _phi[_j][_qp];
    }
    else
    {
        calculateEquilibriumConstant();
