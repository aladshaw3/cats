 ...
 * variables to create a Butler-Volmer redox reaction coupled with temperature, electric potential
 * differences, and sets of 'reduced' and 'oxidized' state variables. Generically, this kernel
 * represents the following reaction schema:
 *
 *                  R --> O + n*e-
 *
 ...
 *
 *            Ref: R. O'Hare, S.W. Cha, W. Colella, F.B. Prinz, Fuel Cell Fundamentals, 3rd Ed.
 * Wiley, (2016) Ch. 3.
 *
 *
...
InputParameters
ModifiedButlerVolmerReaction::validParams()
{
  InputParameters params = Kernel::validParams();
  params.addRequiredParam<std::vector<Real>>("reduced_state_stoich",
                                             "List of stoichiometry for reduced-state reactants");
  params.addRequiredParam<std::vector<Real>>("oxidized_state_stoich",
                                             "List of stoichiometry for oxidized-state products");
  ...
  params.addParam<Real>("electron_transfer_coef",
                        0.5,
                        "Alpha parameter (Default = 0.5 for symmetric electron transfer)");
  ...
  params.addParam<bool>("use_equilibrium_potential",
                        true,
                        "(Default = True) If true, calculates and overrides forward and reverse "
                        "rates based on equilibrium potential");

  params.addParam<Real>("scale", 1.0, "Scaling parameter for this reaction");

  params.addRequiredCoupledVar("reduced_state_vars",
                               "List of names of the reduced-state reactant variables");
  params.addRequiredCoupledVar("oxidized_state_vars",
                               "List of names of the oxidized-state product variables");

  params.addCoupledVar(
      "temperature", 298, "Variable for temperature of the media (default = 298 K)");
  params.addCoupledVar(
      "electric_potential_difference", 0, "Variable for electric potential difference (V or J/C)");
  return params;
}
    ...
    _pot_diff_var(coupled("electric_potential_difference"))
{
  ...
  if (_reduced.size() != _reduced_stoich.size())
  {
    moose::internal::mooseErrorRaw(
        "User is required to provide list of reduced-state reactant variables of the same length "
        "as list of reduced-state reactant stoichiometry.");
  }

  if (_oxidized.size() != _oxidized_stoich.size())
  {
    moose::internal::mooseErrorRaw(
        "User is required to provide list of oxidized-state product variables of the same length "
        "as list of oxidized-state product stoichiometry.");
  }
  ...
  {
    moose::internal::mooseErrorRaw("Electron transfer coefficient must be strictly > 0 and < 1. "
                                   "Default = 0.5 for symmetric electron transfer.");
  }
  for (unsigned int i = 0; i < _oxidized.size(); ++i)
  {
...
Real
ModifiedButlerVolmerReaction::oxidation_rate_fun()
{
  if (useEquilibriumPotential == true)
  {
    return _reaction_rate *
           std::exp(-(1.0 - _alpha) * _equ_pot * _n * _faraday / _gas_const / _temp[_qp]);
  }
  else
  {
...
Real
ModifiedButlerVolmerReaction::reduction_rate_fun()
{
  if (useEquilibriumPotential == true)
  {
...
Real
ModifiedButlerVolmerReaction::oxidation_rate_fun_derivative_with_temp()
{
  if (useEquilibriumPotential == true)
  {
    return _reaction_rate *
           std::exp(-(1.0 - _alpha) * _equ_pot * _n * _faraday / _gas_const / _temp[_qp]) *
           ((1.0 - _alpha) * _equ_pot * _n * _faraday / _gas_const / _temp[_qp] / _temp[_qp]);
  }
  else
  {
    return 0.0;
...
Real
ModifiedButlerVolmerReaction::reduction_rate_fun_derivative_with_temp()
{
  if (useEquilibriumPotential == true)
  {
    return _reaction_rate * std::exp(_alpha * _equ_pot * _n * _faraday / _gas_const / _temp[_qp]) *
           (-_alpha * _equ_pot * _n * _faraday / _gas_const / _temp[_qp] / _temp[_qp]);
  }
  else
  {
    return 0.0;
...
Real
ModifiedButlerVolmerReaction::oxidation_exp_fun()
{
...
Real
ModifiedButlerVolmerReaction::reduction_exp_fun()
{
...
Real
ModifiedButlerVolmerReaction::reduction_state()
{
  Real prod = 1.0;
  for (unsigned int i = 0; i < _reduced.size(); ++i)
  {
...
Real
ModifiedButlerVolmerReaction::oxidation_state()
{
  Real prod = 1.0;
  for (unsigned int i = 0; i < _oxidized.size(); ++i)
  {
...
Real
ModifiedButlerVolmerReaction::reduction_state_without(unsigned int k)
{
  Real prod = 1.0;
  for (unsigned int i = 0; i < _reduced.size(); ++i)
  {
...
Real
ModifiedButlerVolmerReaction::oxidation_state_without(unsigned int k)
{
  Real prod = 1.0;
  for (unsigned int i = 0; i < _oxidized.size(); ++i)
  {
...
Real
ModifiedButlerVolmerReaction::computeQpResidual()
{
...
Real
ModifiedButlerVolmerReaction::computeQpJacobian()
{
  return 0.0;
}

Real
ModifiedButlerVolmerReaction::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _temp_var)
  {
    return -_test[_i][_qp] * _scale * oxidation_rate_fun() * reduction_state() *
               oxidation_exp_fun() *
               (-(1.0 - _alpha) * _n * _faraday * _pot_diff[_qp] / _gas_const / _temp[_qp] /
                _temp[_qp]) *
               _phi[_j][_qp] +
           _test[_i][_qp] * _scale * reduction_rate_fun() * oxidation_state() *
               reduction_exp_fun() *
               (_alpha * _n * _faraday * _pot_diff[_qp] / _gas_const / _temp[_qp] / _temp[_qp]) *
               _phi[_j][_qp] -
           _test[_i][_qp] * _scale * oxidation_rate_fun_derivative_with_temp() * reduction_state() *
               oxidation_exp_fun() * _phi[_j][_qp] +
           _test[_i][_qp] * _scale * reduction_rate_fun_derivative_with_temp() * oxidation_state() *
               reduction_exp_fun() * _phi[_j][_qp];
  }
  if (jvar == _pot_diff_var)
  {
    return -_test[_i][_qp] * _scale * oxidation_rate_fun() * reduction_state() *
               oxidation_exp_fun() * ((1.0 - _alpha) * _n * _faraday / _gas_const / _temp[_qp]) *
               _phi[_j][_qp] +
           _test[_i][_qp] * _scale * reduction_rate_fun() * oxidation_state() *
               reduction_exp_fun() * (-_alpha * _n * _faraday / _gas_const / _temp[_qp]) *
               _phi[_j][_qp];
  }
  for (unsigned int i = 0; i < _reduced.size(); ++i)
  {
    if (jvar == _reduced_vars[i])
    {
      if (_reduced_stoich[i] > 1)
        return -_test[_i][_qp] * _scale * oxidation_rate_fun() * reduction_state_without(i) *
               oxidation_exp_fun() * _reduced_stoich[i] *
               std::pow((*_reduced[i])[_qp], _reduced_stoich[i] - 1.0) * _phi[_j][_qp];
      else
        return -_test[_i][_qp] * _scale * oxidation_rate_fun() * reduction_state_without(i) *
               oxidation_exp_fun() * _reduced_stoich[i] * _phi[_j][_qp];
    }
  }
  for (unsigned int i = 0; i < _oxidized.size(); ++i)
  {
    if (jvar == _oxidized_vars[i])
    {
      if (_oxidized_stoich[i] > 1)
        return _test[_i][_qp] * _scale * reduction_rate_fun() * oxidation_state_without(i) *
               reduction_exp_fun() * _oxidized_stoich[i] *
               std::pow((*_oxidized[i])[_qp], _oxidized_stoich[i] - 1.0) * _phi[_j][_qp];
      else
        return _test[_i][_qp] * _scale * reduction_rate_fun() * oxidation_state_without(i) *
               reduction_exp_fun() * _oxidized_stoich[i] * _phi[_j][_qp];
    }
  }
  return 0.0;
}
