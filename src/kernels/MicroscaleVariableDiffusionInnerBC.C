 ...
 *              of a fictious mesh with variable diffusion coefficients. Each node may have a
 * different diffusivity and thus must have a different variable.
 *
...
InputParameters
MicroscaleVariableDiffusionInnerBC::validParams()
{
...
MicroscaleVariableDiffusionInnerBC::MicroscaleVariableDiffusionInnerBC(
...
Real
MicroscaleVariableDiffusionInnerBC::computeQpResidual()
{
...
Real
MicroscaleVariableDiffusionInnerBC::computeQpJacobian()
{
  _current_diff = _current_diffusion[_qp];
  _upper_diff = _upper_diffusion[_qp];
  _lower_diff = _current_diffusion[_qp];
...
Real
MicroscaleVariableDiffusionInnerBC::computeQpOffDiagJacobian(unsigned int jvar)
{
  _current_diff = _current_diffusion[_qp];
  _upper_diff = _upper_diffusion[_qp];
  _lower_diff = _current_diffusion[_qp];
  ...
  {
    return _test[_i][_qp] * ((_rd_lp1 / _dr / _dr / 2.0) + (_rd_lm1 / _dr / _dr)) * _phi[_j][_qp] *
           (_u[_qp] - _upper_neighbor[_qp]);
  }
  if (jvar == _upper_diff_var)
  {
    return _test[_i][_qp] * ((_rd_lp1 / _dr / _dr / 2.0)) * _phi[_j][_qp] *
           (_u[_qp] - _upper_neighbor[_qp]);
  }
