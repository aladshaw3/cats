...
InputParameters PhaseEnergyTransfer::validParams()
{
    ...
    params.addRequiredCoupledVar("specific_area","Specific area for transfer [surface area of solids / volume solids] (m^-1)");
    return params;
}
...
{
    return _test[_i][_qp] * _hs[_qp] * _specarea[_qp] * _volfrac[_qp] * (_this_temp[_qp] - _other_temp[_qp]);
}

Real PhaseEnergyTransfer::computeQpJacobian()
{
    return 0.0;
}

Real PhaseEnergyTransfer::computeQpOffDiagJacobian(unsigned int jvar)
{
