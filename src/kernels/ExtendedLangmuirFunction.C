 ...
 *K_i * coupled_variable_i / 1 + sum(j, K_j * coupled_variable_j).
 *
...
InputParameters
ExtendedLangmuirFunction::validParams()
{
  InputParameters params = Kernel::validParams();
  params.addParam<Real>(
      "site_density", 0.0, "Maximum Capacity for Langmuir Function of this sorption site (mol/L)");
  params.addParam<std::vector<Real>>(
      "langmuir_coeff", {0}, "Coefficient for the langmuir function (L/mol)");
...
Real
ExtendedLangmuirFunction::computeExtLangmuirEquilibrium()
{
  double sum = 0.0;
  for (unsigned int i = 0; i < _coupled.size(); ++i)
  {
...
Real
ExtendedLangmuirFunction::computeExtLangmuirConcJacobi()
{
  double sum = 0.0;
  ...
  numerator = _langmuir_coef[_lang_index] * _phi[_j][_qp] *
              (1.0 + sum - _langmuir_coef[_lang_index] * _coupled_i[_qp]);
  double denom = (1.0 + sum) * (1.0 + sum);
  return _maxcap * numerator / denom;
}

Real
ExtendedLangmuirFunction::computeExtLangmuirOffJacobi(int i)
{
  double sum = 0.0;
  for (unsigned int j = 0; j < _coupled.size(); ++j)
  {
...
Real
ExtendedLangmuirFunction::computeQpResidual()
{
  return -_test[_i][_qp] * computeExtLangmuirEquilibrium();
}

Real
ExtendedLangmuirFunction::computeQpJacobian()
{
  return 0.0;
}

Real
ExtendedLangmuirFunction::computeQpOffDiagJacobian(unsigned int jvar)
{
  for (unsigned int i = 0; i < _coupled.size(); ++i)
  {
