 ...
 *kernel is particularly useful if you have something like a heterogenous sorption problem and you
 *want track individually the sorbed species on each type of surface site, but also want to couple
...
InputParameters
CoupledSumFunction::validParams()
{
...
CoupledSumFunction::CoupledSumFunction(const InputParameters & parameters) : Kernel(parameters)
{
...
Real
CoupledSumFunction::computeQpResidual()
{
  Real sum = 0.0;
  for (unsigned int i = 0; i < _coupled.size(); ++i)
    sum += (*_coupled[i])[_qp];
...
Real
CoupledSumFunction::computeQpJacobian()
{
  return 0.0;
}

Real
CoupledSumFunction::computeQpOffDiagJacobian(unsigned int jvar)
{
  for (unsigned int i = 0; i < _coupled.size(); ++i)
  {
