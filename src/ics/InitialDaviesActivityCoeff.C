...
InputParameters
InitialDaviesActivityCoeff::validParams()
{
  InputParameters params = InitialCondition::validParams();
  params.addCoupledVar(
      "ionic_strength", 0, "Ionic strength variable for the domain (MUST be in M) [default=0 M]");
  params.addCoupledVar(
      "temperature", 298, "Temperature variable for the domain (MUST be in K) [default=298 K]");
  params.addParam<Real>(
      "fitting_param", 1.82E6, "The Davies model fitting parameter [default=1.82E6]");
  params.addParam<Real>(
      "dielectric_const", 78.325, "Dielectric constant for the media [default=78.325 == Water]");
...
Real
InitialDaviesActivityCoeff::value(const Point & /*p*/)
{
