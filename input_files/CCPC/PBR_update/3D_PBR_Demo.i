...
[Functions]
     [./qc_ic]
         type = ParsedFunction
        value = 4.01E-4*-1.8779*exp(1.8779*z/0.1346)/(1-exp(1.8779))
     [../]

    [./v_ic]
#m/s  avg - superficial velocity (low 2.334 - high 2.4925) ?
        type = ParsedFunction
#value = '2.334 + 0.1585*(z/0.1346)'
...
# Reference or inlet/outlet terms (vary in time, but not in space)
   ...
   [./flow_rate]
       order = FIRST
       family = LAGRANGE       #Must be LAGRANGE if vel_z is also LAGRANGE
...
#NOTE: Because current Navier-Stokes is poorly optimized, it will NOT scale well
#       Need to use ilu or another sparse sub_pc_type for efficiency...
