 ...
 *	\details This file is responsible for estimating various temperature, pressure, and
 concentration *		dependent parameters to be used in other models for gas phase adsorption, mass
 transfer, *		and or mass transport. The goal of this file is to eliminate redundancies in code
 such *		that the higher level programs operate more efficiently and cleanly. Calculations made *
 here are based on kinetic theory of gases, ideal gas law, and some emperical models that *		were
 developed to account for changes in density and viscosity with changes in temperature *		between
 standard temperatures and up to 1000 K.
...
#ifndef Cstd
#define Cstd(p, T)                                                                                 \
  ((p) / (Rstd * T)) ///< Calculation of concentration/density from partial pressure (Cstd = mol/L)
#endif               // Note: can also be used to calculate densities if p = PT * MW

#ifndef CE3
#define CE3(p, T)                                                                                  \
  ((p) / (RE3 * T)) ///< Calculation of concentration/density from partial pressure (CE3 = mol/cm^3)
#endif              // Note: can also be used to calculate densities if p = PT * MW

#ifndef Pstd
#define Pstd(c, T)                                                                                 \
  ((c)*Rstd * T) ///< Calculation of partial pressure from concentration/density (c = mol/L)
#endif

#ifndef PE3
#define PE3(c, T)                                                                                  \
  ((c)*RE3 * T) ///< Calculation of partial pressure from concentration/density (c = mol/cm^3)
#endif

#ifndef Nu
#define Nu(mu, rho)                                                                                \
  ((mu) / (rho)) ///< Calculation of kinematic viscosity from dynamic viscosity and density (cm^2/s)
#endif

#ifndef PSI
#define PSI(T)                                                                                     \
  (0.873143 +                                                                                      \
   (0.000072375 * T)) ///< Calculation of temperature correction factor for dynamic viscosity
#endif

#ifndef Dp_ij
#define Dp_ij(Dij, PT)                                                                             \
  ((PT * Dij) / Po) ///< Calculation of the corrected binary diffusivity (cm^2/s)
#endif

#ifndef D_ij
#define D_ij(MWi, MWj, rhoi, rhoj, mui, muj)                                                       \
  ((4.0 / sqrt(2.0)) * pow(((1 / MWi) + (1 / MWj)), 0.5)) /                                        \
      pow((pow((pow((rhoi / (1.385 * mui)), 2.0) / MWi), 0.25) +                                   \
           pow((pow((rhoj / (1.385 * muj)), 2.0) / MWj), 0.25)),                                   \
          2.0) ///< Calculation of binary diffusion based on MW, density, and viscosity info (cm^2/s)
#endif

#ifndef Mu
#define Mu(muo, To, C, T)                                                                          \
  (muo * ((To + C) / (T + C)) *                                                                    \
   pow((T / To),                                                                                   \
       1.5)) ///< Calculation of single species viscosity from Sutherland's Equ. (g/cm/s)
#endif
...
#ifndef FilmMTCoeff
#define FilmMTCoeff(D, L, Re, Sc)                                                                  \
  ((D / L) * (2.0 + (1.1 * pow(Re, 0.6) *                                                          \
                     pow(Sc, 0.3)))) ///< Calculation of film mass transfer coefficient (cm/s)
#endif
...
typedef struct MIXED_GAS
{
  ...
  MATRIX<double>
      binary_diffusion; ///< Calculated: Tensor matrix of binary gas diffusivities (cm^2/s)
